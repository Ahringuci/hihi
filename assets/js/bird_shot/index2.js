...
import { OBJ } from "./constans2.js";
import * as gml from "../gml.js";

import * as classes from "./classes2.js";
...
(function () {
    ...
    const GameSettings = () => {
        ...
        const getHighscore = localStorage.getItem("bird_highscore2");
        if (getHighscore) {
    ...
    const BirdInit = () => {
        ...
        let _xOry = gml.choice(0, 1),
            ...
            _size = gml.irandom_range(6, 40),
            _speedx = gml.random_range(0.5, 5),
            _speedy = gml.random_range(0.5, 5);
    ...
    const Update = () => {
        ...
        if (o_notification.length !== 0) {
            OBJ.o_notification = o_notification.filter(
                (noti) => noti.state !== "destroy"
            );
            o_notification.map((noti) => noti.update());
        }
        if (bloods.length !== 0) {
        ...
        gml.draw_text(
            ...
            10,
            60,
            "Birds: " + birds.length.toString() + "/15",
            "#fff"
        );
        ...
        if (OBJ.birds.length > 15 || OBJ.bulletAmount <= 0) {
            OBJ.state = "destroy";
        }
    };
